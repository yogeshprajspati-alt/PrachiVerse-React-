...
const MockTest = ({ questions, storageKey, testTitle }) => {
    ...
    // Storage key helpers
    const stageKey   = `${storageKey}_stage`;
        ...
        if (stage === 'test' && !isSubmitted) {
            timerRef.current = setInterval(() => {
                setTimeRemaining(prev => {
                    ...
                    localStorage.setItem(timeKey, prev - 1);
                    return prev - 1;
                });
            }, 1000);
        }
    ...
    useEffect(() => {
        if (stage === 'test') {
            ...
            localStorage.setItem(statusKey, JSON.stringify(questionStatus));
                                ...
                                <div key={subj} style={{ display: currentSubject === subj ? 'block' : 'none' }}>
                                    ...
                                    <div className={styles.questionGrid}>
                                        {questions[subj].map((q, idx) => {
                                            ...
                                            else if (s === 'visited') statusClass = styles.qStatusNotAnswered;
