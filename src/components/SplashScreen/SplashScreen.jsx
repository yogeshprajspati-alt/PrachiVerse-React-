...
const getTextContainerVariants = (isLowPower) => ({
    hidden: { opacity: 0 },
    visible: {
        opacity: 1,
        transition: {
            staggerChildren: isLowPower ? 0.04 : 0.08,
            delayChildren: isLowPower ? 0.1 : 0.3
        }
    }
});
...
const SplashScreen = ({ onComplete }) => {
    ...
    // Fix 4: Smooth lightweight letter animation for low-power mode (no heavy blur/spring)
    const simplifiedLetterVariants = {
        ...
        visible: {
            ...
            transition: { 
                duration: 0.4,
                ease: [0.25, 0.1, 0.25, 1] // Custom ease-out for a snappy feel
            }
        }
    };
        ...
        <AnimatePresence>
            {isVisible && (
                ...
                >
                    ...
                    <motion.div
                        variants={getTextContainerVariants(isLowPower)}
                        initial="hidden"
                        animate="visible"
                        style={{
                            display: 'flex',
                        ...
                        {charsArray.map((char, index) => (
                            <motion.span
                                ...
                                style={{
                                    ...
                                    fontSize: 'clamp(1.5rem, 6.5vw, 5.5rem)',
                                    ...
                                    backgroundClip: 'text',
                                    textShadow: (!isLowPower && char.trim()) ? '0 15px 25px rgba(0,0,0,0.6)' : 'none',
                                    filter: isLowPower ? 'none' : 'drop-shadow(0 0 15px rgba(236, 72, 153, 0.4))',
                                    paddingBottom: '10px'
                                }}
                            >
                                {char === ' ' ? '\u00A0' : char}
                    ...
                    <motion.div
                        ...
                        transition={{ delay: isLowPower ? 0.7 : 1.5, duration: 0.8, ease: "easeOut" }}
                        style={{
                    ...
                    <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        transition={{ delay: isLowPower ? 0.9 : 2.2, duration: 0.8 }}
                        style={{
                            marginTop: '1.5rem',
